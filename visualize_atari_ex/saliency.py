...
prepro = lambda img: np.array(Image.fromarray(img[35:195].mean(2)).resize((80, 80))).astype(np.float32).reshape(1,
                                                                                                                80,
                                                                                                                80) / 255.
...
def score_frame(model, history, ix, r, d, interp_func, mode='actor'):
    ...
    scores = np.array(Image.fromarray(scores).resize((80, 80), resample=Image.BILINEAR)).astype(np.float32)
